	...
	void lithash( lit str){
		...
		for(int k=0; k<str.lebal.length(); k++)
			if(str.lebal[k]!='=' && str.lebal[k]!='C' && str.lebal[k]!='c' && str.lebal[k]!='X'&& str.lebal[k]!='x'&& (int)str.lebal[k]!=39)
				lit=lit+str.lebal[k];
		int ip =imp.sum(str.lebal)%100; //算位置 
...
/*		for(int i=0; i<grp.size(); i++){
		...
		for(int i=0; i<grp.size(); i++, ip++){
				...
				for(int j=0; j<opc.size();j++){
					...
					for (int m=0; m <opc[j].name.length(); m++){  
						if((int)opc[j].name[m]>=97 && (int)opc[j].name[m]<=122) //a~z 
	       					key[m] = toupper(key[m]);  //小寫變大寫 
	       				else
	       					key[m] = tolower(key[m]); // 大寫變小寫  
					}	
			...
			//	cout<<"name:"<<grp[i].op1.name<<endl;
				for(int m=0; m<i && grp[i].op1.name!="\0"; m++){  //這個lebal之前有出現過 
					if(grp[m].label.name==grp[i].op1.name){ //m之前出現  i現在 operand
						lebaladdr=mc[m].addr;
						break;
					}
				}
			...
			//	cout<<"lebaladdr: "<<lebaladdr<<endl;
				...
				if(lebaladdr=="\0"&&i!=0){
					string ok="f";
					for(int m=0; m<cft.size()&&grp[i].op1.name[0]!='='; m++){ //operand已在組態表裡 
...
string caldisp(string ta, string pc){
	...
	if(tmp>0){ //ta-pc(10進位)=tmp ->  轉2 ->3 nibber(12 bits)要補滿 -> 轉16
	...
	else{  //轉2-> pc 的 1 to 0，0 to 1 後 +1 (結果設為af) -> af和ta相加(2進位) ->  轉16
			//3 nibber(12 bits)要補滿 
